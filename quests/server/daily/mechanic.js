...
new game.DailyQuest({
	...
	who: 'mechanic',
	name: 'Хищная рыба', 
	text: 'Хэй, Консул. Последние достижения оборонной и наступательной техники и оружия — моя стихия. Но иногда в озере появляется вторая хищная рыба. Ты понимаешь, Консул? Им становится ужасно неудобно сосуществовать. У меня есть координаты второй рыбы — мелкий торговец и шарлатан, ничего серьезного. Никакого насилия, просто припугнешь его флотом. Естественно не за «просто так». Что скажешь?',
	...
	answers: { 
		...
		newsoldiers: { 
			text: 'Отправлю новобранцев, все равно они бесполезны.',
			win: 'Я рад, что мы оба извлекли выгоду из этой ситуации.',
			fail: 'Новобранцы растерялись и превратили моего конкурента и его корабль в фарш. Это не то, о чем я просил, Консул.' 
		},
		oneship: {
			text: 'Возьми один крейсер.',
			win: 'Я сомневался в твоем плане, но результат меня полностью устроил. Оказалось, что этот торговец — тот еще трус.',
...
new game.DailyQuest({
	...
	who: 'mechanic',
	name: 'Астероид', 
	text: 'Привет, Консул. Мои источники сообщили, что на днях мимо твоей колонии пройдет астероид. На нем есть небольшие залежи крайне редкого и сверхпрочного минерала — чакнорисцит. Ходят легенды, что он выкристаллизовался из пота великого героя прошлого. Твои инженеры помогут мне с добычей, а я обещаю хорошо заплатить.', 
	answers: { 
		gopstop: { 
			text: 'Забирай вон тех, что сидят на корточках.', 
			win: 'Команда, конечно, поначалу выглядела неубедительно, но оказалось, что дело свое знает. Вот твоя часть, Консул.', 
			fail: 'Увы, Консул, мы три часа пытались растолкать заниженный крейсер, слушали хиты Вити М-158, сквозь тонировку всматривались в темные глубины космоса, но в итоге пропустили астероид.' 
		},
		okgo: { 
			text: 'Хорошо, я подготовлю команду инженеров к вылету.',
			win: 'Все прошло как нельзя лучше, Консул. Приятно иметь деловые отношения с хорошими партнёрами.',
			fail: 'Минерал оказался гораздо тверже, чем я предполагал, Консул. Нам не удалось достать его из недр астероида.' 
		},
		chuckisback: {
			text: 'По рукам. Это же просто камень, что может случиться?',
			fail: 'Консул, это поврежденное послание — все, что осталось от твоей команды. Неловко вышло. "Пшш… Достань меня. Достань меня, крутой парень… Пшш… Покажи, на сколько ты крут. И вот это значит добывать? Пшш… Вот что значит добывать! Пшш…'
		}
	}
});
...
new game.DailyQuest({
	...
	text: 'Как дела, Консул? Я не всегда играю по правилам Совета, поэтому меня недолюбливают патрули и при первой же возможности стараются обыскать мое судно. Другое дело — сам Консул. Они даже не станут досматривать твой корабль. Мне пришла одна очень важная, но не совсем законная посылка. Твоя задача доставить ее до места назначения. По рукам?', 
	answers: { 
		goodguy: { 
			text: 'Я не собираюсь перевозить на своем корабле краденые железки.', 
			fail: 'Хм, ну что же, это твое право, Консул. Поищу кого-нибудь другого.' 
		},
		ultradestroyer: { 
			text: 'Привезти какую-то коробку не составит труда.',
			win: 'Все прошло просто превосходно. Посылка у меня, вознаграждение — у тебя. Все честно.',
		...
		imnotapostman: {
			text: 'Я же не почтальон! Этим займется кто-нибудь из моей команды.',
			win: 'Нам повезло, Консул: патруль уже направлялся к транспортному кораблю, как вдруг какой-то недоумок открыл пальбу по пролетающим мимо судам и отвлек внимание на себя. Это было очень рискованно.',
			fail: 'Как я и думал, первый же патруль обнаружил на борту нашего транспортного корабля мою посылку. Кажется, тебе придется объясниться перед Советом.'
		}
	}
});
...
new game.DailyQuest({
	...
	answers: { 
		letsdoit: { 
			...
			win: 'Кажется, все прошло гладко. Датчики не пострадали, информация у меня. Да и тебе пару буровых насадок сэкономили.', 
		...
		test: {
			text: 'Давай сначала протестируем твой порошок на чем-нибудь другом.',
...
new game.DailyQuest({
	...
	answers: { 
		...
		takemyship: { 
			text: 'Можешь взять один из кораблей и сам все сделать.',
			win: 'Наглость — второе счастье, Консул. Но в этом случае я даже доволен таким решением. Сделаю все тихо и без лишних глаз.',
...
new game.DailyQuest({
	...
	answers: { 
		...
		soundsgood: { 
			text: 'Вперед! Искать в космосе приключения на свою задницу!',
			win: 'Затея действительно была рискованная, но, на мой вкус, окупилась с лихвой. Вот твоя часть, Консул.',
			fail: 'Жаль, что всей нашей добычей стал аварийный передатчик. Не расстраивайся, Консул, в следующий раз повезет.' 
		},
		gogopowerconsuls: {
...
new game.DailyQuest({
	...
	answers: { 
		...
		raceismysecondname: {
			text: 'В детстве я упал в котел с гонками.',
			win: 'Ну что же, Консул, ты действительно неплохо водишь. До лидеров заезда, конечно, далеко, но призовые деньги ты все же захватил. Вот они, кстати.',
			fail: 'А, Консул. Я рад, что ты наконец-то добрался домой. Трое суток для преодоления трассы — внушительное достижение.'
		}
	}
});
...
new game.DailyQuest({
	...
	text: 'Привет, Консул. Ко мне обратился мой старый знакомый. Он занимал один из постов в экономическом блоке Совета и работал, как оказалось, не всегда честно. Теперь Совет желает «просто поговорить» с ним. Совет начал прочесывать колонии, и твоя — последняя в списке. Если ты предоставишь ему убежище на несколько дней, то получишь неплохое вознаграждение.', 
	answers: { 
		cleft: { 
			text: 'Идет, вот координаты расселины на юге Колонии.', 
			win: 'Он просил передать слова благодарности и вот эту посылку. Вряд ли мы услышим о нем вновь.', 
			fail: 'Когда он приземлился в указанном месте, дикие животные напали на него и разорвали бедолагу в клочья. К сожалению, трупы не выписывают счета, Консул.' 
		},
		iamtheking: { 
			text: 'Не царское это дело преступников прятать.',
			fail: 'Это твое решение, Консул. Я найду заинтересованного в доходе человека.' 
		},
		godfather: {
...
new game.DailyQuest({
	...
	text: 'Хей, Консул. У меня есть деловое предложение. Знакомая мне группа инженеров разрабатывает новую технологию для гипердвигателей. Обещают многократное повышение надежности и КПД. Загвоздка в том, что они независимые разработчики, и сейчас им необходимы технологические помещения для выполнения предварительных тестов. Если все пройдет успешно, то с продажи прототипа мы сможем заработать.', 
	answers: { 
		barn: { 
			text: 'Гони их вон в тот амбар.', 
			win: 'Все прошло как нельзя лучше. Экспериментальные данные оказались настолько обнадеживающими, что нашелся инвестор. Вот твоя часть с продажи, Консул.', 
		...
		sorrynope: {
			...
			win: 'Буквально вчера узнал, что они нашли все-таки место для исследований. Правда, не на долго: через полчаса после начала прогремел взрыв.',
...
new game.DailyQuest({
	...
	text: 'Консул, недалеко от твоей Колонии был замечен новый разведывательный корабль рептилоидов. Судя по моей информации он совсем один. На этом судне установлены передовые образцы радиозащиты, и я не могу упустить шанс заполучить их. Ресурсы меня не интересуют, так что я готов отдать все содержимое корабля в качестве платы за боевую поддержку.', 
	answers: { 
		newepisode: { 
			text: 'Ты с ума сошел? Сегодня же новая серия выходит.', 
		...
		killsomereptiles: { 
			...
			win: 'Как я и предполагал, этот корабль стал легкой добычей. Надеюсь, ты доволен содержимым трюмов?',
		...
		quiet: {
			...
			win: 'Это было действительно впечатляюще. Маленькие пушистые засранцы знают свое дело. На моей памяти так ловко и тихо еще никто не работал.',
			fail: 'Опрометчивое решение, Консул. Я же сказал, что это новейший корабль рептилоидов с передовыми экспериментальными системами защиты и обнаружения. Твои еноты даже не заметили, как превратились в пыль вместе с кораблем.'
		}
	}
});
