...
new game.QuestLine({
	...
	new game.Quest({
		...
		options: {
			accept: {
				text: 'Факт! Без домино даже Консул работать не будет.',
				mood: 'positive'
			}
		},
		isDone: function() {
		...
		options: {
			accept: {
				text: 'А куда дели старушку-то? Ась?',
				mood: 'positive'
			}
		},
		isDone: function() {
		...
		engName: 'metal_70',
		text: '<p>О, Консул, у меня хорошие новости о Шахте Металла. Бурильный Бур так далеко ушёл вниз, что взрывоопасный газ больше не поступает на поверхность, а выгорает прямо во время переработки. На всякий случай Инженеры рекомендовали поставить в ствол Шахты э-э-э… «датчики уровня газодинамической активности» и организовать диспетчера, но в целом, похоже, опасность миновала. Персонал теперь может спокойно работать на поверхности без респираторов. С вашего разрешения я начну выписывать оборудование для новой диспетчерской службы, а инженеры займутся монтажом.</p>',
		...
		options: {
			accept: {
				text: 'У всех есть занятие! Пойду, флот отправлю куда-нибудь.',
				mood: 'positive'
			}
		},
		isDone: function() {
