initBuildingsContentResidential = function () {
...
new game.Building({
	...
	effect: [new Game.Effect.Income({
		...
		result: function(level) {
			level = (level !== undefined ? level : this.level); 
			return [0, 100000, 250000, 500000, 1000000, 2000000][Math.floor(level / 20)];
		}
	})],	
...
new game.Building({
	...
	effect: [new Game.Effect.Income({
		...
		result: function(level) {
			level = (level !== undefined ? level : this.level); 
			return [0, 25000, 300000, 5000000, 2000000, 50000000][Math.floor(level / 20)];
		}
	})],
	basePrice: function(level) {
			level = level || this.currentLevel();
...
new game.Building({
	...
	effect: [new Game.Effect.Income({
		...
		result: function(level) {
			level = (level !== undefined ? level : this.level); 
			return [0, 10000, 100000, 1000000, 5000000, 15000000][Math.floor(level / 20)];
		}
	})],
	basePrice: function(level) {
			level = level || this.currentLevel();
...
new game.Building({
	...
	basePrice: function(level) {
			...
			var price = {
				metals:   [375, Game.functions.slowExponentialGrow, 0],
				crystals: [195, Game.functions.slowExponentialGrow, 0]
			};
...
new game.Building({
	...
	effect: [new Game.Effect.Special({
		pretext: 'Время между турнирами ',
		aftertext: '',
		result: function(level) {
			level = (level !== undefined ? level : this.level);
			return level === 0
				? 0
				: Game.Helpers.formatSeconds( Game.Building.special.Colosseum.getCooldownPeriod(level) );
		}
	}), new Game.Effect.Special({
		pretext: '',
	...
	basePrice: function(level) {
			...
			if (level < 20) {
				...
				price.garyoldmanium = [5, Game.functions.slowLinearGrow, 60];
			} else {
	...
	requirements: function(level) {
		...
		if (level < 20) {
			...
			return [
				...
				[Game.Building.items.residential.statue,        45],
			];
		} else {
			return [
				[Game.Research.items.evolution.alloy,          100],
				[Game.Building.items.military.factory,          80],
				[Game.Research.items.evolution.crystallization, 75],
				[Game.Building.items.residential.statue,        75]
			];
		}
	}
});
...
new game.Building({
	...
	basePrice: function(level) {
			...
			var price = {
				metals:   [2500, Game.functions.slowExponentialGrow, 0],
				crystals: [3000, Game.functions.slowExponentialGrow, 0]
			};

			if (level > 19) {
				price.honor = [80, Game.functions.slowLinearGrow, 20];
			}
			...
			if (level < 20) {
				...
				price.ancient_artefact = [3, Game.functions.slowLinearGrow, 60];
			} else {
				price.ruby_plasmoid = [8, Game.functions.slowLinearGrow, 80];
			}
			return price;
	...
	requirements: function(level) {
		...
		if (level < 20) {
			...
			return [
				...
				[Game.Building.items.military.oscd,         35],
			];
		} else {
			return [
				...
				[Game.Building.items.military.oscd,         55]
			];
		}
	}
});
...
new game.Building({
	...
	basePrice: function(level) {
			...
			if (level < 20) {
				...
				price.chip = [6, Game.functions.slowLinearGrow, 20];
			} else if (level < 60) {
				price.nicolascagium = [5, Game.functions.slowLinearGrow, 40];
			} else if (level < 80) {
				price.ancient_artefact = [3, Game.functions.slowLinearGrow, 60];
			} else {
				price.ruby_plasmoid = [8, Game.functions.slowLinearGrow, 80];
	...
	requirements: function(level) {
		...
		if (level < 20) {
			...
			return [
				...
				[Game.Building.items.military.complex,      85]
			];
		}
	}
});
