	...
	func: function(power, unit, enemies) {
		...
		for (key in enemies) {
			...
			if (canAttack) {
				...
				enemies[key].count = Math.ceil( enemies[key].life / enemies[key].characteristics.life );
...
new game.BattleEffect({
	...
	func: function(chance, count, unit, target) {
		...
		target.life = target.count * target.characteristics.life;
...
new game.BattleEffect({
	...
	func: function(unit) {
		...
		unit.damage = unit.characteristics.damage.max * unit.count;
...
new game.BattleEffect({
	...
	func: function(chance, unit, target) {
		...
		var unitCount = Math.ceil( target.hits[unit.key] / unit.characteristics.damage.max );
		...
		var damage = critCount * target.characteristics.life;
		target.life -= damage;
		target.hits[unit.key] += damage;
...
new game.BattleEffect({
	...
	func: function(chance, unit, targets) {
		...
		var randKey = null;
		var threshold = unit.characteristics.damage.max * 0.1;
...
new game.BattleEffect({
	...
	func: function(unit, targets) {
		...
		var damage = Math.floor( unit.characteristics.damage.max * 0.4 );
...
new game.BattleEffect({
	...
	func: function(chance, unit, target) {
		...
		target.count -= count;
		target.life -= count * target.characteristics.life;
...
new game.BattleEffect({
	...
	func: function(value, unit, targets) {
		...
		for (var key in targets) {
			...
			targets[key].count = Math.ceil( targets[key].life / targets[key].characteristics.life );
