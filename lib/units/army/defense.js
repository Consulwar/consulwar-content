...
new game.Unit({
	...
	engName: 'bomb',
	description: 'Мины прекрасное средство против кораблей среднего размера, огромные минные поля останавливаются вдали от планеты в приоритетных местах для варп прыжка. Крупные корабли без проблем могут выдерживать урон мин, зато корабли поменьше будут уничтожены сразу по прибытию.',
...
new game.Unit({
	...
	engName: 'ionbomb',
	description: 'О, это очень крутая штука, Консул. Ионная мина магнитом притягивается к кораблям противника, после детонации создаётся несколько волн воздействия на корабли Рептилоидов, первая полна пробивает щиты, вторая прошивает корпус, а третья повреждает электронику. Отличный повод застроить купол из таких мин вокруг своей планеты.',
	effect: [new Game.Effect.Special({
...
new game.Unit({
	...
	engName: 'turret',
	description: 'Эти специализированные Турели, Консул, устанавливаются на дальних рубежах, они не требуют перезарядки, заряжаются от солнечной энергии, и имеют угол обзора 360 градусов, за счёт своих гравитационных двигателей. В основном используются для уничтожения небольших и шустрых кораблей.',
	effect: [new Game.Effect.Special({
...
new game.Unit({
	...
	engName: 'laserturret',
	description: 'Лазерные Турели это следующая модификация обычных турелей, принцип действия тот же. Находясь в отдалённой точке космоса, эти орудия способны крайне быстро и эффективно уничтожать небольшие корабли противника, которые умудрились подобраться слишком близко к вашей планете. Очень полезная штука, Консул.',
	effect: [new Game.Effect.Special({
...
new game.Unit({
	...
	engName: 'snipergun',
	description: 'Такие орудия как Снайпер Ган и Рельсовая Пушка - устанавливаются на специальных платформах на орбите вашей планеты, они способны вести прицельный, точечный огонь по хорошо бронированным кораблям. Дальность атаки позволяет этому орудию не подвергаться нападении некоторое время после начала боя.',
	effect: [new Game.Effect.Special({
...
new game.Unit({
	...
	engName: 'railcannon',
	description: 'Рельсовая Пушка просто необходима для любителей ваншот тактики уничтожения противника. Корабли средней мозности Рептилоидов разлетают в щепки с одного выстрела такого орудия как Рельсовая Пушка. Эти орудия действительно спосбны перевернуть ход битвы в вашу сторону за несколько выстрелов.',
	effect: [new Game.Effect.Special({
...
new game.Unit({
	...
	description: 'Мелкие и шустрые корабли Чешучатых постоянно обходят вашу оборону, крупные орудия не могут по ним попасть, а у простеньких пушек недостаточно урона, чтобы справиться с тысячами Истребителей? Это больше не проблема. Плазменный Убийца распидорасит в щепки этих надоедливых мошек. Его скорость атаки и урон просто превосходны.',
	effect: [new Game.Effect.Special({
...
new game.Unit({
	...
	engName: 'tyrant',
	description: 'Когда Рептилии нападают на вас мощным флотом и выстраивают серьёзную защиту из крупных и тяжёлых кораблей, они уж точно никак не расчитывают что вы будите поливать из жидкой плазмой. Эта новейшая разработка в области вооружения, способна словно ножь в масло, прорезать корабли ваших врагов насквозь. ПотКристаллрясающее орудие.',
	effect: [new Game.Effect.Special({
...
new game.Unit({
	...
	engName: 'crystalgun',
	description: 'Кристалл-Ган это потрясающий пример оборонных технологий. Сам комплекс орудий устанавливается на планете, посему уничтожить его довольно проблематично, при этом дальность атаки и точность этих орудий позволяют наносить огромный урон кораблям Зелёных тварей. Причём концентрированная энергия кристаллов способна прошибать броню самых крупных кораблей.',
	effect: [new Game.Effect.Special({
...
new game.Unit({
	...
	engName: 'trilinear',
	description: 'Вершина эволюции обороны, Мощность этого орудия просто колоссальная, его концентрированная кристаллизированная плазма заставляет получить моментальный загар тысячи Рептилоидов на бортах крупных кораблей. А щиты этих орудий могут выдерживать огромное количество урона. Разрушить эти орудия практически невозможно.',
	effect: [new Game.Effect.Special({
...
new game.Unit({
	...
	// Сигнатуры нет
	engName: 'doomsdaygun',
	description: 'Раз, два, орудие судного дня заберёт тебя... Это не шутки, Консул. Это превосходное строение способно уничтожать целые планеты и всё что находится рядом с ними. Орудия настолько мощные, что на их перезарядку уёдт куча времени. И всё же, Консул, будьте осторожны, это оружие способно изменить ход истории.',
	effect: [new Game.Effect.Special({
