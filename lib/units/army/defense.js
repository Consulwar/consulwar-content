...
new game.Unit({
	...
	battleEffects: [{
		pretext: 'Спецспособность: принимают на себя ',
...
new game.Unit({
	...
	battleEffects: [{
		pretext: 'Спецспособность: шанс нанести двойной урон ',
		aftertext: '%',
		result: function(level) {
			return 10;
		},
		onRoundStart: function(unit, friends, enemies, round, options) {
			...
			return Game.Unit.battleEffects.multiplyDamage(2, unit);
		}
	}],
	basePrice: {
		metals: 3000,
	...
	battleEffects: [{
		...
		onAttack: function(unit, friends, enemies, round, options) {
			...
			for (var i = 0; i < keys.length; i++) {
				var key = keys[i];
				if (enemies[key] && enemies[key].life > 0) {
					return Game.Unit.battleEffects.redirectDamageTo(1, unit, enemies[key], enemies);
				}
			}
			return false;
		}
...
new game.Unit({
	...
	battleEffects: [{
		pretext: 'Спецспособность: шанс не получить урон в первом раунде ',
		aftertext: '%',
		result: function(level) {
			return 20;
		},
...
new game.Unit({
	...
	battleEffects: [{
		pretext: 'Спецспособность: после боя восстанавливается ',
...
new game.Unit({
	...
	battleEffects: [{
		pretext: 'Спецспособность: Годзилла получает ',
		aftertext: '% урона',
		result: function(level) {
			return 150;
		},
...
new game.Unit({
	...
	battleEffects: [{
		pretext: 'Спецспособность: Тень получает ',
		aftertext: '% урона',
		result: function(level) {
			return 200;
		},
		...
		onAttack: function(unit, friends, enemies, round, options) {
			...
			for (var key in friends) {
				if (friends[key].group == 'defense') {
					var message = Game.Unit.battleEffects.reduceDamage(50, friends[key]);
					if (message) {
						result.push(message);
					}
				}
