	...
	effect: [{
		...
		*/
		onAttack: function(unit, friends, enemies, round, options) {
			...
			return 'Гаммадрон принимает на себя урон ' + total;
		}
	}],
	//})],
	basePrice: {
...
new game.Unit({
	...
	onDeath: function(unit, friends, enemies, round, options) {
		// Должно срабатывать когда убиты все? Или когда убит один?
	}
});
...
new game.Unit({
	...
	triggers: {
		battle: [{
			name: 'test',
			applyEffect: function(unit, friends, enemies, round, options) {
				if (round != 1 || Game.Random.interval(1, 99) > 30) {
					return false;
				}
				...
				unit.hits = {};
				return 'Мираж не получает урона';
			}
		}]
	}
});
	...
	triggers: {
		battle: [{
			...
			applyEffect: function(unit, friends, enemies, round, options) {
				if (!enemies['blade']
				 || !enemies['blade'].hits
				 || !enemies['blade'].hits['frigate']
				) {
					return false;
				}

				var bonusDamage = Math.floor( enemies['blade'].hits['frigate'] * 0.15 )
				enemies['blade'].life -= bonusDamage;
				enemies['blade'].hits['frigate'] += bonusDamage;
				return 'Фрегат наносит дополнительный урон клинку ' + bonusDamage;
				// Дполнительный урон? Или вычитается из общего значения damage?
			}
		}]
	}
});
