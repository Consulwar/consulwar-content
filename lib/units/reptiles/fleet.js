...
new game.ReptileUnit({
  ...
  battleEffects: [{
    pretext: 'во время первой атаки бьет только в одну цель, но со ',
    aftertext: '% урона',
    result: function(level) {
      return 150;
    },
...
new game.ReptileUnit({
  ...
  battleEffects: [{
    notImplemented: true,
    pretext: 'во время первой атаки получает на ',
...
new game.ReptileUnit({
  ...
  battleEffects: [{
    pretext: 'наносит урон сразу по 3-м целям, и всем по ',
    aftertext: '% урона',
    result: function(level) {
      return 100;
    },
