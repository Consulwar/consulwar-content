...
new game.Research({
	...
	basePrice: function(level) {
			...
			if (level > 19) {
				price.honor = [100, Game.functions.slowLinearGrow, 20];
			}
...
new game.Research({
	...
	basePrice: function(level) {
			...
			if (level > 19) {
				price.honor = [110, Game.functions.slowLinearGrow, 20];
			}
			...
			if (level < 20) {
				...
				price.meteor_fragments = [4, Game.functions.slowLinearGrow, 40];
			} else if (level < 80) {
...
new game.Research({
	...
	basePrice: function(level) {
			...
			if (level > 19) {
				price.honor = [120, Game.functions.slowLinearGrow, 20];
			}
			...
			if (level < 20) {
				...
				price.secret_technology = [4, Game.functions.slowLinearGrow, 60];
			} else {
				price.quad_cooler = [6, Game.functions.slowLinearGrow, 80];
			}	
			return price;
	},
	maxLevel: 100,
	requirements: function(level) {
		...
		if (level < 20) {
			...
			return [
				[Game.Building.items.military.laboratory,  25],
				[Game.Research.items.evolution.ikea,       20]
			];
		} else if (level < 60) {
			return [
				[Game.Building.items.military.laboratory,  35],
...
new game.Research({
	...
	basePrice: function(level) {
			...
			if (level > 19) {
				price.honor = [130, Game.functions.slowLinearGrow, 20];
			}
			...
			if (level < 20) {
				...
				price.nanowires = [6, Game.functions.slowLinearGrow, 80];
			}	
			return price;
	},
	maxLevel: 100,
...
new game.Research({
	...
	basePrice: function(level) {
			...
			if (level > 19) {
				price.honor = [140, Game.functions.slowLinearGrow, 20];
			}
			...
			if (level < 20) {
				...
				price.rotary_amplifier = [5, Game.functions.slowLinearGrow, 40];
			} else if (level < 80) {
...
new game.Research({
	...
	basePrice: function(level) {
			...
			if (level < 20) {
				...
				price.emerald_plasmoid = [4, Game.functions.slowLinearGrow, 40];
			} else if (level < 80) {
				price.sapphire_plasmoid = [6, Game.functions.slowLinearGrow, 60];
			} else {
...
new game.Research({
	...
	engName: 'engineering',
	description: 'Злоебучие Рептилоиды повсюду, Консул. Следует держать ухо в остро, а ещё лучше - заштамповаться оборонительными сооружениями. Создать из своей планету целую непокорную крепость с сотнями.. нет... Тысячами! Орудий! Чёрт, как возбуждает. Но вы же понимаете, Консул, что оборону нужно развивать, ничего само собой с неба не падает, кроме поджареных рептилоидов... если у вас есть планетарные пушки.',
	...
	basePrice: function(level) {
			...
			if (level > 19) {
				price.honor = [250, Game.functions.slowLinearGrow, 20];
			}
			...
			if (level < 20) {
				...
				price.chip = [6, Game.functions.slowLinearGrow, 40];
			} else if (level < 80) {
				price.keanureevesium = [4, Game.functions.slowLinearGrow, 60];
			} else {
				price.ancient_technology = [3, Game.functions.slowLinearGrow, 80];
			}	
			return price;
	},
	maxLevel: 100,
...
new game.Research({
	...
	basePrice: function(level) {
			...
			if (level > 19) {
				price.honor = [200, Game.functions.slowLinearGrow, 20];
			}
			...
			if (level < 20) {
				...
				price.nanowires = [6, Game.functions.slowLinearGrow, 40];
			} else if (level < 80) {
	...
	requirements: function(level) {
		...
		if (level < 20) {
			...
			return [
				[Game.Building.items.military.laboratory,  70],
				[Game.Research.items.evolution.ikea,       65]
			];
		} else {
			return [
...
new game.Research({
	...
	effect: [new Game.Effect.Special({
		pretext: 'Максимальная скорость полета ',
		aftertext: ' ае / ч',
		result: function(level) {
			level = (level !== undefined ? level : this.level);
			return Game.Planets.calcMaxSpeed(level) * 60 * 10;
		}
	}), new Game.Effect.Special({
		pretext: 'Ускорение ',
		aftertext: ' ае / ч',
		result: function(level) {
			level = (level !== undefined ? level : this.level);
			return Game.Planets.calcAcceleration(level) * 60 * 60 * 10;
		}
	})],
	basePrice: function(level) {
			level = level || this.currentLevel();
			...
			if (level > 19) {
				price.honor = [300, Game.functions.slowLinearGrow, 20];
			}
			...
			if (level < 20) {
				...
				price.amethyst_plasmoid = [4, Game.functions.slowLinearGrow, 60];
			} else {
				price.ancient_knowledge = [4, Game.functions.slowLinearGrow, 80];
			}	
			return price;
	},
	maxLevel: 100,
	requirements: function(level) {
		...
		if (level < 20) {
			...
			return [
				[Game.Building.items.military.laboratory,       55],
				[Game.Research.items.evolution.crystallization, 20]
			];
		} else if (level < 60) {
			return [
				[Game.Building.items.military.laboratory,       65],
			...
			return [
				[Game.Building.items.military.laboratory,       75],
				[Game.Research.items.evolution.crystallization, 60]
			];
		} else {
			return [
				[Game.Building.items.military.laboratory,       85],
				[Game.Research.items.evolution.crystallization, 80]
			];
		}
	}
});
...
new game.Research({
	...
	basePrice: function(level) {
			...
			if (level < 20) {
				...
				price.ancient_artefact = [3, Game.functions.slowLinearGrow, 80];
			}	
			return price;
	},
	maxLevel: 100,
...
new game.Research({
	...
	basePrice: function(level) {
			...
			if (level < 20) {
				...
				price.keanureevesium = [4, Game.functions.slowLinearGrow, 40];
			} else if (level < 80) {
	...
	requirements: function(level) {
		...
		if (level < 20) {
			...
			return [
				[Game.Building.items.military.laboratory,      85],
				[Game.Research.items.evolution.nanotechnology, 80],
...
new game.Research({
	...
	description: 'ОСД или, как её прозвали ребята из научного сообщества: Орудие Судного Дня - это новейшая система вооружения, требующая огромное количество энергии для использования. В тоже время испытания на одной из планет показали на что способно ОСД. К сожалению, орудие настолько мощное, что приходится тратить кучу времени на его зарядку и калибровку. Однако же результат стоит того в полной мере.',
	effect: [new Game.Effect.Special({
		notImplemented: true,
	...
	basePrice: function(level) {
			...
			if (level > 19) {
				price.honor = [800, Game.functions.slowLinearGrow, 20];
			}
	...
	requirements: function(level) {
		...
		if (level < 20) {
			...
			return [
				[Game.Building.items.military.laboratory,      80],
				[Game.Research.items.evolution.hyperdrive,     70],
				[Game.Research.items.evolution.nanotechnology, 65]
			];
		} else if (level < 80) {
			return [
				[Game.Building.items.military.laboratory,      90],
				[Game.Research.items.evolution.hyperdrive,     80],
				[Game.Research.items.evolution.nanotechnology, 75]
			];
		} else {
			return [
				[Game.Building.items.military.laboratory,     100],
				[Game.Research.items.evolution.hyperdrive,     90],
				[Game.Research.items.evolution.nanotechnology, 85],
				[Game.Research.items.evolution.converter,      80]
			];
		}
	}
