...
new game.Research({
	...
	effect: [new Game.Effect.Special({
		notImplemented: true,
		pretext: 'Время временной карточки +',
		aftertext: '%',
		result: function(level) {
			level = (level !== undefined ? level : this.level); 
			return level * 0.5;
...
new game.Research({
	...
	effect: [new Game.Effect.Military({
		pretext: 'Броня оборонных сооружений +',
		aftertext: '%',
		condition: {
			type: 'unit',
			group: 'defense'
...
new game.Research({
	...
	effect: [new Game.Effect.Special({
		pretext: 'Шанс добыть карту в бою +',
		notImplemented: true,
		aftertext: '%',
		result: function(level) {
			level = (level !== undefined ? level : this.level); 
...
new game.Research({
	...
	effect: [new Game.Effect.Special({
		notImplemented: true,
		pretext: 'Удвоить плазмоиды преобразователем +',
		aftertext: '%',
		result: function(level) {
			level = (level !== undefined ? level : this.level); 
			return level * 0.7;
